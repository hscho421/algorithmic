...
export const TEMPLATES = {
  min_coins: {
    ...
    code: [
      { line: 'def coin_change(coins, amount):', indent: 0 },
      { line: 'dp = [float("inf")] * (amount + 1)', indent: 1 },
      { line: 'dp[0] = 0', indent: 1 },
      { line: '', indent: 0 },
      { line: 'for i in range(1, amount + 1):', indent: 1 },
      { line: 'for coin in coins:', indent: 2 },
      { line: 'if i >= coin:', indent: 3 },
      { line: 'dp[i] = min(dp[i], dp[i - coin] + 1)', indent: 4 },
      { line: '', indent: 0 },
      { line: 'return dp[amount] if dp[amount] != float("inf") else -1', indent: 1 },
    ],
  },
  count_ways: {
    id: 'count_ways',
    ...
    code: [
      { line: 'def count_ways(coins, amount):', indent: 0 },
      { line: 'dp = [0] * (amount + 1)', indent: 1 },
      { line: 'dp[0] = 1', indent: 1 },
      { line: '', indent: 0 },
      { line: 'for coin in coins:', indent: 1 },
      { line: 'for i in range(coin, amount + 1):', indent: 2 },

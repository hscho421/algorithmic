...
export default function SelectionSortVisualizer() {
  const [arrayInput, setArrayInput] = useState('29, 10, 14, 37, 13');
  const {
    items: savedInputs,
    isLoading: savedLoading,
    saveInput,
    deleteInput,
  } = useSavedInputs('selection-sort');
  ...
  const {
    ...
    saveCheckpoint,
    deleteCheckpoint,
  } = useProgress('selection-sort', progressPayload);
  ...
  const variables = state
    ? [
        ...
        { name: 'minIdx', value: state.minIdx ?? '—', desc: 'current minimum index' },
      ]
    : [];
        ...
        },
        {
          ...
          content: (
            <div className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
              <p><span className="text-zinc-900 dark:text-white font-medium">1. Find:</span> Scan unsorted portion for minimum</p>
              <p><span className="text-zinc-900 dark:text-white font-medium">2. Swap:</span> Swap minimum with first unsorted element</p>
              <p><span className="text-zinc-900 dark:text-white font-medium">3. Repeat:</span> Expand sorted portion by one</p>
              <p className="text-xs text-zinc-500 mt-4">Always O(n²) comparisons, but minimizes swaps!</p>
            </div>
          ),
        },
        ...(result

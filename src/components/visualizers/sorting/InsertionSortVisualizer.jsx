...
export default function InsertionSortVisualizer() {
  ...
  const {
    ...
    saveCheckpoint,
    deleteCheckpoint,
  } = useProgress('insertion-sort', progressPayload);
  ...
  const variables = state
    ? [
        ...
        { name: 'shifts', value: state.shifts || 0, desc: 'element shifts' },
        { name: 'key', value: state.key ?? '—', desc: 'current key' },
      ]
    : [];
        ...
        },
        {
          ...
          content: (
            <div className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
              <p><span className="text-zinc-900 dark:text-white font-medium">1. Pick:</span> Take the next unsorted element as "key"</p>
              <p><span className="text-zinc-900 dark:text-white font-medium">2. Shift:</span> Move larger sorted elements to the right</p>
              <p><span className="text-zinc-900 dark:text-white font-medium">3. Insert:</span> Place key in its correct position</p>
              <p className="text-xs text-zinc-500 mt-4">Efficient for small or nearly-sorted arrays!</p>
            </div>
          ),
        },
        ...(result

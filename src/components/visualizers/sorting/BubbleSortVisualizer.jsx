...
export default function BubbleSortVisualizer() {
  ...
  const {
    ...
    saveCheckpoint,
    deleteCheckpoint,
  } = useProgress('bubble-sort', progressPayload);
  ...
  const variables = state
    ? [
        ...
        { name: 'pass', value: state.pass ?? '—', desc: 'current pass' },
      ]
    : [];
        ...
        },
        {
          ...
          content: (
            <div className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
              <p><span className="text-zinc-900 dark:text-white font-medium">1. Compare:</span> Compare adjacent elements</p>
              <p><span className="text-zinc-900 dark:text-white font-medium">2. Swap:</span> If out of order, swap them</p>
              <p><span className="text-zinc-900 dark:text-white font-medium">3. Repeat:</span> Continue until no swaps needed</p>
              <p className="text-xs text-zinc-500 mt-4">Best case O(n) when already sorted!</p>
            </div>
          ),
        },
        ...(result

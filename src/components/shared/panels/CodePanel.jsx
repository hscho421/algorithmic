...
const highlightSyntax = (line) => {
  ...
  const functions = ['mergeSort', 'merge', 'insert', 'search', 'inorder', 'preorder', 'postorder', 'levelorder', 'visit', 'enqueue', 'dequeue'];
  ...
  return (
    <span>
      {tokens.map((token, idx) => (
        <span
          key={idx}
          className={`${isActive ? 'text-amber-100' : isPast ? 'opacity-60' : ''} ${!isActive ? getTokenColor(token.type) : ''}`}
        >
          {token.value}
        </span>
      ))}
      ...
      <div className="font-mono text-sm">
        {(title || description) && (
          <div className="mb-4 pb-3 border-b border-zinc-800">
            {title && <h3 className="text-zinc-300 font-semibold">{title}</h3>}
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${isActive ? 'bg-amber-500/20 border-l-2 border-amber-500' : 'border-l-2 border-transparent'}
                `}
              >
                <span className={`w-6 text-right mr-4 text-xs ${isActive ? 'text-amber-400' : 'text-zinc-600'}`}>
                  {idx + 1}
                </span>

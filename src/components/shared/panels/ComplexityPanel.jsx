      ...
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-zinc-800/50 rounded-lg p-3 border border-zinc-700/50">
            ...
            <div className="font-mono text-emerald-400">{time}</div>
          </div>
          <div className="bg-zinc-800/50 rounded-lg p-3 border border-zinc-700/50">
            <div className="text-xs text-zinc-500 uppercase tracking-wider mb-1">Space</div>
            <div className="font-mono text-blue-400">{space}</div>
          </div>
        </div>
          ...
          <div className="space-y-2 pt-2 border-t border-zinc-800">
            {bestCase && (
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono text-sm text-emerald-400">{bestCase}</span>
              </div>
            )}
            {averageCase && (
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-500">Average Case</span>
                <span className="font-mono text-sm text-amber-400">{averageCase}</span>
              </div>
            )}
            {worstCase && (
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-500">Worst Case</span>
                <span className="font-mono text-sm text-rose-400">{worstCase}</span>
              </div>
            )}
          </div>
        )}
